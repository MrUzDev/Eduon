...
export default function FilteredCourses() {
  ...
  useEffect(() => {
    ...
    if (initialValue === "+absolut_price") {
      // setFilterType('+absolut_price')
      setTitle("Narx yuqorilab borishi bo'yicha kurslar");
    } else if (initialValue === "-absolut_price") {
      // setFilterType('-absolut_price')
      setTitle("Narx pasayib borishi bo'yicha kurslar");
    } else  {
      setTitle("Top kurslar");  
    }
    try {
      ...
      axios
        .get(
          `${process.env.REACT_APP_API_KEY}/api/v2/courses/filter/?ordering=${
        ...
        .then((res) => {
          setcourseData(
            initialValue === "top"
              ? res.data.filter((item) => item.is_top == true)
              : res.data
          );
          seterrorShowBool(false);
        })
