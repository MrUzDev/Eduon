...
export default function VideoUpload(props) {
  ...
  const [aboutVideo, setAboutVideo] = useState(EditorState.createEmpty());
  ...
  const finalUpload = async (e) => {
    ...
    try {
      ...
      console.log(`${process.env.REACT_APP_STREAM_API}/media/upload`);
      await axios
  ...
  const uploadAll = (url, duration) => {
    ...
    try {
      ...
      aboutVideo && formData.append("about", draftToHtml(convertToRaw(aboutVideo.getCurrentContent())));
  ...
  useEffect(() => {
    try {
      axios
        .get(
          `${process.env.REACT_APP_API_KEY}/api/v2/courses/module/${courseId}`
        )
        .then((res) => {
      ...
      axios
        .put(
          `${process.env.REACT_APP_API_KEY}/api/v1/courses/lesson/${lessonId}`,
          {
            ...
            subtitle_url: null,
            about: draftToHtml(convertToRaw(aboutVideo.getCurrentContent())),
            duration: duration,
            module: moduleId,
              ...
              <h2 className="miniTitle">Video tavsifi</h2>
               ...
               <Editor
                          toolbar={{inline: {className: 'UploadEditorDesk'}}}
                            editorState={aboutVideo}
                            toolbarClassName="toolbarClassName"
                            wrapperClassName="wrapperClassName"
                            editorClassName="editorClassName"
                            onEditorStateChange={onChangeAboutVideo}
                          />
            </div>
          </div>
