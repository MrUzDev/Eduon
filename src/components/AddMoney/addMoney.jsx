...
export default function AddMoney(props) {
  ...
  // const [accountBlance, setAccountBlance] = useState();
  ...
  const [mtsSum, setMtsSum] = useState();
  const [ISpeaker, setISpeaker] = useState(false);
  const [showTransVoucherModal, setShowTransVoucherModal] = useState(false);
  const [voucherTransferSum, setVoucherTransferSum] = useState();
  const [voucherTransferPhone, setVoucherTransferPhone] = useState();
  const [transferCardType, setTransferCardType] = useState('')
  ...
  const transfer = async () => {
    if ((savedCardUID || cardNum) && amout && expire) {
      const headers = {
  ...
  const saveCardfunc = (e) => {
    console.log("Saved card");
    try {
      axios
        .post(
          `${process.env.REACT_APP_API_KEY}/api/v1/wallet/card`,
          {
  ...
  const MtcTransaction = async () => {

    if (mtsSum) {
      await axios
        .post(
          `${process.env.REACT_APP_API_KEY}/api/v2/wallet/mts-payment`,
          {
            amount: parseInt(mtsSum),
          },
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("access")}`,
        ...
        .then((res) => {
          res.data.result
            ? mtcUrlTransition(res.data.result.transfer.debit.form_url)
            : dataError();
        })
  ...
  const VoucherTransder = async () => {
    if (
      vaucherBlanceData &&
      vaucherBlanceData.balance / 100 >= voucherTransferSum &&
      voucherTransferSum &&
      voucherTransferPhone &&
      voucherTransferPhone.length == 12
    ) {
      try {
        await axios.post(
          `${process.env.REACT_APP_API_KEY}/api/v2/wallet/transfer-voucher`,
          {
            amount: parseInt(voucherTransferSum),
            receiver_phone: parseInt(voucherTransferPhone),
          },
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("access")}`,
        ...
        ).then((res) => {setShowTransVoucherModal(false); VoucherTransferSuccess()})
      } catch (error) {}
    } else {
      !(vaucherBlanceData.balance / 100 >= voucherTransferSum) && transferVoucherSumError();
      !(voucherTransferSum && voucherTransferPhone) && addMoneyError();
      voucherTransferPhone.length !== 12 && PhoneNumError()
    }
  };
  ...
  const mtcUrlTransition = (link) => {
    window.location.href = link;
  };

  // useEffect(() => {
  //   console.log(savedCardUID);
  // }, [savedCardUID]);
  // useEffect(() => {
  ...
  const MtcSumAlert = () => toast.error("Iltimos biron bir summani kiriting.");
  const dataError = () => toast.error("Error");
  const PhoneNumError = () => toast.error("Raqam to'liq emas");
  const transferVoucherSumError = () =>
    toast.error("Hisobingizdagi voucher balansi yetarli emas!");
  const addMoneyError = () =>
    toast.error("Iltimos, ma'lumotlarni to'liq kiriting!");
  const VoucherTransferSuccess = () => toast.success("Muvafaqiyatli o'tkazildi");
      ...
      <div className="container">
        <div className="rowGrid">
          <div className="col-10 col-lg-10 col-sm-24">
            ...
            {vaucherBlanceData && vaucherBlanceData.balance !== 0 && (
              <div className="cardBalans">
                <div className="icon">
                  <div className="img">
                    <svg width="50" height="50" viewBox="0 0 50 50" fill="none">
                  ...
                  <div className="uzs">
                    <div className="smSvg">
                      <p>Hisobdagi vaucherlar balansi</p>
                      <h1>
                      ...
                      <p>
                        Yaroqlik muddati{" "}
                        {moment
                          .utc(vaucherBlanceData.expire_date)
                          .format("Do - MMMM YYYY")}
                      </p>
                    </div>
                  </div>
            ...
            {/* {ISpeaker && ( */}
              <div className="transfer-Voucher-Container">
                <button
                  className="eduon-brand-btn"
                  onClick={setShowTransVoucherModal}
                >
                  Voucherni o'tkazish 
                </button>
              </div>
            ...
            >
              {props.savedCards.map((item, index) => (
                ...
                key={index}
                onClick={() => {
                  if (savedCardUID === item.card_uuid) {
                    setCardNum("");
                    setExpire("");
                    ...
                    <div className="saved">
                      ...
                      {item.type == "virtual" && (
                        ...
                        />
                         ...
                         {item.type == "virtual" && (
                      <h6>Balans: {currency(item.balance / 100, "UZS")
          ...
          <div className="col-14 col-lg-14 col-sm-24">
            <div className="cardRegister">
              <div className="addMoneyTitle d-flex justify-between">
                <h1 className="smMarginTop smPay">Hisobni to’ldirish</h1>
                <div>
                  <button
                    className="outline-btn"
                    onClick={() => setMtcOpen(true)}
                  >
                    Rossiya kartasi orqali to'ldirish
                  </button>
                </div>
              </div>
              {error && (
          ...
          >
            <strong>
              O'tkazmani amalga oshirish muvaffaqiyatsiz yakunlandi!!!
            </strong>
            <br />
          </Alert>
          <Alert
          ...
          <Modal
            isOpen={mtcOpen}
            onRequestClose={() => setMtcOpen(false)}
            style={customStyles}
          ...
          >
            ...
            <div style={{ background: "#fff", marginBottom: "20px" }} className="voucherTransfer">
              <h4 id="addMoneyMtcTitle">To’lov summasi</h4>
              <p style={{ marginBottom: "20px" }}>Minimum 15.000 so'm</p>
              <div className="password">
              <TextField
                className="inputs invisibleBg phoneNumVoucherTrans"
                id="TransparentInput"
                sx={{
                  width: "100%",
                  "& .MuiOutlinedInput-notchedOutline": {
              ...
              />
                 <p id="inputNum">
                  ...
                  <h5 className="eye">UZS</h5>
              </div>
            </div>
            <button id="fillBtnMtc" onClick={MtcTransaction}>
              O’tkazmani tasdiqlash
            </button>
          </Modal>
          ...
          >
            ...
            <div style={{ background: "#fff", marginBottom: "20px" }} className="voucherTransfer">
              <h4 id="addMoneyMtcTitle">Voucherni boshqa hisobga o'tqazish.</h4>
              ...
              <TextField
                ...
                onChange={(e) => {
                  setVoucherTransferPhone(e.target.value);
                }}
              />
              <br />
              <br />
              ...
              <TextField
                ...
                type="number"
                label="Qancha o'tkazmoqchisiz"
                variant="outlined"
                autoComplete='off'
                onChange={(e) => {
                  setVoucherTransferSum(e.target.value);
                }}
              />
               <p id="inputNum">
                    {voucherTransferSum &&
                      currency(voucherTransferSum, "UZS")
                        .replace("UZS", "")
                        .replace("soʻm", "").replace(/,/g, ".").slice(0, -3)
                        }
                  </p>
            ...
            <button id="fillBtnMtc" onClick={VoucherTransder}>
              O’tkazmani tasdiqlash
            </button>
          </Modal>
        </div>
