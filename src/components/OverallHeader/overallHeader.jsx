...
const Ranges = [
  {
    ...
    value: [startOfDay(new Date(moment().format())), endOfDay(new Date(moment().format()))],
  },
  {
    label: "Kecha",
    value: [
      startOfDay((new Date(moment().subtract(1, 'days').format()))), endOfDay((new Date(moment().subtract(1, 'days').format())))
    ],
  },
  {
    label: "7 kun oldin",
    value: [startOfDay((new Date(moment().subtract(7, 'days').format()))), endOfDay(new Date(moment().format()))],
  },
];
...
function OverallHeader() {
  ...
  const { navStretch, balance, myEnrolledCourses, setmyEnrolledCourses, vaucherBlanceData } =
  ...
  const [startDate, setStartDate] = useState(moment().subtract(30, 'days').format().slice(0,10)); /// 05.12.22
  // const [startDate, setStartDate] = useState(2023-02-23); /// 05.12.22
  ...
  useEffect(() => {
    try {
      axios
        ...
        .then((res) => {
          let filteredVebinars = res.data.filter(
            (item) => item.is_valid === "ON HOLD"
          );
          setStreamProcess(filteredVebinars);
        });
    } catch (error) {}
  }, []);
  ...
  // post useEffect👇
  useEffect(() => {
    statisEnrolledPost(startDate, endDate).then((data) =>
  ...
  // get useEffect👇
  useEffect(() => {
    StudentRecevidMoney(startDate, endDate).then((data) => {setReceived(data.data.amount);});
    statisRatingPost(startDate, endDate).then((data) => setRating(data.data.rating));

    statisChartAmout(startDate, endDate).then((res) =>{setChartAmout(res.data);})
  ...
  const onDateChange = (dates) => { 
    ...
    statisRatingPost(startDate, endDate).then((data) =>
  ...
  useEffect(() => {
    setChartData(chartAmout && Object.entries(chartAmout).map((val) => ({
      name: moment.utc(val[0]).format('Do - MMMM'),
      sum: (val[1] / 100)
    })))
  }, [chartAmout])
  ...
  return (
    <div className="statistic">
      <div className="headerRow rowGrid justify-between align-center">
        ...
        <div className="col-6 col-lg-12 col-sm-24 mb-lg-30">
          <div className="box">
            ...
            <div className="hover-box">
              <p>Sizning umumiy balansingiz</p>
            </div>
          </div>
        </div>
        <div className="col-6 col-lg-12 col-sm-24 mb-lg-30">
          <div className="box">
            ...
            <div className="hover-box">
              <p>Sizning umumiy o'quvchilaringiz soni</p>
            </div>
          </div>
        </div>
        <div className="col-6 col-lg-12 col-sm-24 mb-lg-30">
          <div className="box"> 
            ...
            <div className="hover-box">
              <p>Sizning umumiy kurslaringiz soni</p>
            </div>
          </div>
        </div>
        <div className="col-6 col-lg-12 col-sm-24 mb-lg-30">
          <div className="box">
            ...
            <div className="hover-box">
              <p>umumiy kurslaringizga ro'yxatdan o'tishlar soni</p>
            </div>
          </div>
        </div>
      </div>
      <div className="rowGrid">
          ...
          <div className="profit_box">
            <div className="boxes">
              ...
              <div className="profit">
                <h3>
                  {received &&
                  currency(received / 100, "UZS").replace("UZS", "").replace("soʻm", "").replace(/,/g, ".").slice(0, -3)}
                  &nbsp;so'm
                </h3>
                {Math.ceil((startDate - endDate) / 1000) === 604800 ||
                Math.ceil((startDate - endDate) / 1000) === -604800 ? (
                  <p>bu haftada</p>
                ) : null}
              </div>
        ...
        <div className="col-10 col-lg-24 col-sm-24 mt-lg-30">
          <p className="title">Jarayondagi Kurslarim va Vebinarlarim</p>
          {coursesOnProcess?.length || streamProcess.length != 0 ? (
            <>
                ...
                <div
                  ...
                  <span>
                    <img
                      ...
                      alt="#"
                    />
                    <p>{item.name && item.name.length > 10 ? item.name.slice(0, 20) + '...  ' : item.name}</p>
                  </span>
                </div>
              ))}
            </>
